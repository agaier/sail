...
if doPlot
    if ~meshOnly
        ...
        h=fPlot([cTop(1,1:end/2); -cTop(2,1:end/2)],'k'); h.LineWidth = 3;
        h = title('Z Curve (4 Params)');axis equal;grid on;
        h.FontSize = 14;
        subplot(Yview);
        ...
        h=title('Y Curves (7 Params)');axis equal;grid on;
        h.FontSize = 14;
        subplot(Xview);
        h=fPlot([cRibBase(2,:); cRibBase(3,:)]);  
        h.LineWidth = 3; h.Color = [0.7525 0.7384 0.3768]; axis square;grid on;
        h=title('X Curve (5 Params)');
        h.FontSize = 14;
        subplot(allSides);
    end
        v = v.*scale; v(:,3) = v(:,3) +.2;
        h = patch('Faces',f,'Vertices',v,...
            'EdgeColor','black','FaceColor','green','LineWidth',.1,'FaceAlpha',.25);
        view([-44.4, 26.6]);axis equal; grid on;
        %% Plot Component Lines
        hold on;
        h = plot3(cTop(1,1:end/2).*scale,cTop(2,1:end/2).*scale,cTop(3,1:end/2).*scale);
        h.LineWidth = 5; h.Color = 'k';
        
        h = plot3(cTop(1,1:end/2).*scale,-cTop(2,1:end/2).*scale,cTop(3,1:end/2).*scale);
        h.LineWidth = 5; h.Color = 'k';
        
        h = plot3(cMid(1,:).*scale,cMid(2,:).*scale,cMid(3,:).*scale+.2);
        h.LineWidth = 5; h.Color = 'b';
        
        h = plot3(cRidge(1,:).*scale,cRidge(2,:).*scale,cRidge(3,:).*scale+.2);
        h.LineWidth = 5; h.Color = 'r'; h.LineStyle = ':';
        
        h = plot3(cRidge(1,:).*scale,-cRidge(2,:).*scale,cRidge(3,:).*scale+.2);
        h.LineWidth = 5; h.Color = 'r'; h.LineStyle = ':';
        
        h = plot3(-cBot(1,:).*scale,cBot(2,:).*scale,-cBot(3,:).*scale+.2);
        h.LineWidth = 5; h.Color = 'm'; h.LineStyle = '-';
        
        for i=1:length(RibSection)-1
            cRib = RibSection{i};
            h = plot3(cRib(1,:).*scale,cRib(2,:).*scale,cRib(3,:).*scale+.2);
            h.LineWidth = 5; h.Color = [0.7525 0.7384 0.3768];
            h = plot3(cRib(1,:).*scale,-cRib(2,:).*scale,cRib(3,:).*scale+.2);
            h.LineWidth = 5; h.Color = [0.7525 0.7384 0.3768];
        end
