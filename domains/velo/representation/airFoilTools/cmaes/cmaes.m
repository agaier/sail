...
%             end
            ...
            openFoamFolderBase = [varargin{:} 'case'];
            parfor iInd = 1:size(arxvalid,2)
                fit(iInd) = feval(fitfun, arxvalid(:,iInd),...
                    [openFoamFolderBase int2str(iInd) '/constant/triSurface/parsecWing.stl'],...
                    [openFoamFolderBase int2str(iInd) '/']);
            end
            fitness.raw = fit;
        ...
        for k=find(isnan(fitness.raw))
            ...
            while isnan(fitness.raw(k)) && isempty(stopflag)
                ...
                openFoamFolder = [varargin{:} 'case1/'];
                fit(k) = feval(fitfun, arxvalid(:,k),...
