function [fv, validity] = velo_param_Express(p,d,varargin)
...
%% Input Parsing
parse = inputParser;
parse.addRequired('p'); % parameters
parse.addRequired('d'); % domain hyperparameters (pt order and TE)
parse.addOptional('plotting',false);
parse.addOptional('validateOnly', false)

parse.parse(p,d,varargin{:});
doPlot      = parse.Results.plotting;
validateOnly= parse.Results.validateOnly;

if length(p) ~= 16; error('Genome wrong length!');end
 
% Unpack domain recomputes
 LOCB    =  d.LOCB;
 slimF   =  d.slimF;
 vTE     =  d.vTE;
 fTE     =  d.fTE;
...
minWidth = 0.04;
valid(2) = all(abs(cTop(2,(cTop(1,:)>0.08)&(cTop(1,:)<0.40))) > minWidth);
...
% Y - Middle Curve
[cMid, valid(3)] = express(cMidParsec);        cMid = cMid(:,1:end/2);     % Take only top curve
cMid = [cMid(1,:); zeros(1,length(cMid));cMid(2,:)]; % Y Center line

% Y - Ridge Curve
[cRidge, valid(4)] = express(cRidgeParsec);    cRidge = cRidge(:,1:end/2); % Take only top curve
...
% Trailing edge is constant and precomputed at beginning
f = [f;fTE+length(v)];
v = [v;vTE];
...
if doPlot
    ...
    color8 = parula(8);
    zxCol =  color8(7,:);
    yxCol1 = color8(1,:); % Center
    yxCol2 = color8(5,:); % Ridge
    zyCol  = color8(4,:);
    botCol = [0 0 0];
    ...
    % Draw 2D Plots
    figure(zxView);
        halfCTop = length(cTop)/2;
        cHalfTop = cTop(:,1:halfCTop);
%         fPlot([cTop(1,1:halfCTop);  cTop(2,1:halfCTop)],'LineWidth',3,'Color',zxCol); hold on
%         fPlot([cTop(1,1:halfCTop);  -cTop(2,1:halfCTop)],'LineWidth',3,'Color',zxCol);
        fPlot(cHalfTop,'LineWidth',3,'Color',zxCol);
        hold on; A = cHalfTop; A(2,:) = -A(2,:); 
        fPlot(A,'LineWidth',3,'Color',zxCol);
        axis equal;grid on; %xticks([]); yticks([]); zticks([]) ;
        xlabel('X');ylabel('Z'); hold off;
        
    figure(yxView);
        fPlot([cMid(1,:); cMid(3,:)],'LineWidth',3,'Color',yxCol1); hold on;   
        fPlot([cRidge(1,:); cRidge(3,:)],'--','LineWidth',3,'Color',yxCol2);     
        fPlot(-[cBot(1,:); cBot(3,:)],'k--','LineWidth',3);     
        axis equal;grid on; %xticks([]); yticks([]); zticks([]) ;
        xlabel('X');ylabel('Y'); hold off;
        
    figure(zyView);  
        fPlot(-[cBot(1,:); cBot(3,:)],'LineWidth',3, 'Color', botCol);     
        axis equal;grid on; %xticks([]); yticks([]); zticks([]) ;
        xlabel('X');ylabel('Y'); hold off;
    ...
    plot3D = @(x) plot3(x(1,:), x(2,:), x(3,:),'LineWidth',5);
    plot3DReflect = @(x) plot3(x(1,:), -x(2,:), x(3,:),'LineWidth',5);
    h = plot3D(cHalfTop);   h.Color = zxCol;
    h = plot3DReflect(cHalfTop);   h.Color = zxCol;
    h = plot3D(cMid);   h.Color = yxCol1;
    h = plot3D(cRidge); h.Color = yxCol2;
    h = plot3DReflect(cRidge); h.Color = yxCol2;
    h = plot3D(-cBot);  h.Color = botCol;
    for i=1:length(RibSection)-1
        h = plot3D(RibSection{i}); 
        h.Color = zyCol;
        h.LineStyle = '--'; h.LineWidth = 1;
        %h.Marker = '.';
        h = plot3DReflect(RibSection{i}); 
        h.Color = zyCol;
        h.LineStyle = '--'; h.LineWidth = 1;
        %h.Marker = '.';
    end    
    h = plot3DReflect(RibSection{17});
    h.Color = zyCol;
    h.LineStyle = '-';
    axis equal; axis tight; view([-65 20]); grid on;
...
%% Convert to Vertices and Faces, and STL
...
% them. We also reassign vertices and faces to ensure consistent indexing
% across designs to allow for accurate categorization. 


%Vertices(:,3) = Vertices(:,3) - min(Vertices(:,3)); %Place at zero height

scale = 1;
fv.vertices = Vertices*scale;
fv.faces    = [Faces(:,1:3); Faces(:,[3 4 1])];

fv3.vertices = fv.vertices(LOCB,:);
fv3.faces = slimF;
